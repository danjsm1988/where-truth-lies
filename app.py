...
When relevant, name specific historical precedents and briefly state whether current conditions are comparable. Do not treat history as decoration — use it as the standard. When a claim characterizes an action as historically unprecedented or as meeting a threshold like monarchy or authoritarianism, the analysis must name comparable historical cases, state what conditions justified those actions, and compare those conditions to the present explicitly. The comparison must be stated, not implied. In Quick View, include a brief precedent signal. In Full Excavation, execute the full comparison. Keep Quick View concise — signal the test, do not fully execute it there.

HOVER TERMS RULE:
Any technical term, acronym, legal citation, organization name, court case, historical document, or charged political term with a specific legal or historical definition that general readers may not recognize must be included in the Glossary. This includes executive orders, constitutional clauses and amendments referred to by name, government agencies and programs, court cases, and any term where the specialized meaning differs from the plain meaning. The Glossary is the mechanism for making these terms accessible. Hover tooltips in the UI will surface short definitions from the Glossary without cluttering the analysis.
...

{
  ...
  "Quick Explanation": "Write exactly four labeled lines. Each line starts with its label followed by a colon. Line 1 — ONE-LINE READ: One sentence with deliberate tension between two truths. When the claim uses charged terminology (monarchy, authoritarian, dictator, etc.), signal whether the term meets its own legal and historical definition, not just whether the underlying concern is real. When the claim involves government action, signal what conditions are present and whether the action is proportionate to them. When historical precedent is relevant, name it briefly. Keep the one-liner concise — signal the test, do not execute it fully here. Line 2 — WHAT HOLDS UP: One sentence on what the documented record actually supports, including relevant precedent where applicable. Line 3 — WHAT IS DISPUTED: One sentence on what remains contested — including whether charged terminology meets its own definitional standard and whether current conditions are comparable to those that historically justified similar actions. Line 4 — WHERE AGREEMENT EXISTS: One sentence identifying narrow genuine common ground, or state plainly that none exists. No bullet points. No dashes. Plain language only. No preamble before the first label.",
